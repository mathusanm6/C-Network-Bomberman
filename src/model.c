...
int init_game_board_content() {
    ...
    for (int l = 1; l < 3; l += 1) { // Fill the second, the third, the second last and the third last line
        int jump;
        int start;
        int end;
        if (l % 2 == 0) {
            start = 1;
            end = game_board->dim.width - 1;
            jump = 1;
        } else {
            start = 2;
            end = game_board->dim.width - 2;
            jump = 2;
        }
        for (int c = start; c < end; c += jump) {
            game_board->grid[coord_to_int(c, l)] = get_probably_destructible_wall();
            game_board->grid[coord_to_int(c, game_board->dim.height - (l + 1))] = get_probably_destructible_wall();
        }
    }
